...
np.save(dir + 's_obs.npy', s_obs)
print(s_obs)

# Sample from null distribution via the bootstrap.
reps = 1000
ss = np.zeros(reps)
LPX = torch.log(marginal_X(X, flow_0.log_prob))
LPY = torch.log(marginal_Y(Y, flow_0.log_prob))
for i in range(reps):
    ...
    idx = np.random.choice(n, size = n, replace = False)
    X_boot = D[idx, 0].reshape(-1, 1)
    D_boot = torch.cat([X_boot, Y], dim = 1)

    # Compute the information coefficient.
    ss[i] = flow_0.log_prob(D_boot).mean() - LPX.mean() - LPY.mean() 
    print(i, '\t', ss[i])
np.save(dir + 'ss.npy', ss)

# Compute the p value.
s_p_val = np.mean(ss >= s_obs)
np.save(dir + 's_p_val.npy', s_p_val)
print(s_p_val)