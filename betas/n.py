...
b = 1
a = 1
nn = 10**np.linspace(1, 6, 6).astype(int)
for n in nn:
    # Create data
    ...
    os.makedirs('n/{}/XY'.format(b), exist_ok = True)
    train_flow(flow_XY, optimizer_XY, D, 'n/{}/XY/'.format(b))
    
    # Train X flow
    flow_X = make_flow(d = 1)
    flow_X.to(device)
    optimizer_X = optim.Adam(flow_X.parameters())
    os.makedirs('n/{}/X'.format(b), exist_ok = True)
    train_flow(flow_X, optimizer_X, D, 'n/{}/X/'.format(b))
    
    # Train Y flow
    flow_Y = make_flow(d = 1)
    flow_Y.to(device)
    optimizer_Y = optim.Adam(flow_.parameters())
    os.makedirs('n/{}/Y'.format(b), exist_ok = True)
    train_flow(flow_Y, optimizer_Y, D, 'n/{}/Y/'.format(b))